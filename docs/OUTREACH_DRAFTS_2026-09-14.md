# Outreach drafts for approval — Monday 14 Sep 2026
...
**Price?**
```
Trial first (14 days). After that either Unlimited (~£22/mo) or Match Desk Pass packs — pick at signup. Detail on the site.
```
...
## B. Reddit — value-first (no cold DMs)

Suggested first subs (confirm before posting): `r/sportsbroadcasting`, `r/broadcasting`. Soft on `r/soccer` (comments only if on-topic).

### B1 — Post (help-first, soft CTA)
...
## C. Facebook — group / page

Needs: which groups Chris is in. Until then, keep as Page/personal post drafts.
...
```
*(Attach homepage trailer when posting.)*
...
## Day-1 rhythm (suggested)

| Channel | Action |
|---------|--------|
| LinkedIn | 10–15 connect notes (A1/A1b) after Chris approves templates + first names |
| Reddit | Post **one** of B1 in an approved sub (or B3 comments only) |
